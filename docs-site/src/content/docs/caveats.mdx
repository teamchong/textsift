...
- Pre-commit / CI gating to block commits or PRs that introduce PII (via the [pre-commit hook](/textsift/precommit/) and [GitHub Action](/textsift/github-action/)).
- Generating realistic test fixtures from prod data (via [Faker mode](/textsift/playground-faker/)) so downstream code keeps working.
