// End-to-end Dawn-direct forward pass using synthetic weights.
//
// Mirrors tests/native/forward-metal.js exactly but uses the Dawn-direct
// backend (hand-written GLSL → SPIR-V via the dawn* NAPI surface). Same
...
//
// Use this to bench Dawn-direct end-to-end at any T.
...
// and the Dawn port collapses them into one push-constant block.
...
export class DawnForward {
  constructor(opts = {}) {
    this.cfg = { ...PF, ...opts };
    this.backend = native.dawnCreateBackend();
  ...
  _addBuf(name, bytes) {
    ...
    const ptr = native.dawnCreateBuffer(this.backend, buf);
  ...
  _uploadWeights() {
    ...
    console.log(
      `[DawnForward] weights uploaded: ${this.weights.size} tensors, ` +
  ...
  ensureScratch(maxT) {
    ...
    if (this.scratch) {
      for (const ptr of Object.values(this.scratch)) native.dawnReleaseBuffer(this.backend, ptr);
    }
    this.maxT = maxT;
    const cfg = this.cfg;
    const D = cfg.hiddenSize;
    ...
    const F16 = 2, F32 = 4;
    const T = maxT;
    const empty = (bytes) => native.dawnCreateEmptyBuffer(this.backend, Math.max(16, (bytes + 3) & ~3));
    this.scratch = {
      idsBuf: empty(T * 4),
      maskBuf: empty(T),
  ...
  forward(tokenIds, attentionMask) {
    ...
    native.dawnWriteBuffer(this.backend, s.idsBuf, 0, tokenBytes);
    const maskPadded = new Uint8Array(Math.max(4, (T + 3) & ~3));
    maskPadded.set(attentionMask);
    native.dawnWriteBuffer(this.backend, s.maskBuf, 0, maskPadded);
    ...
    native.dawnWriteBuffer(this.backend, s.cosBuf, 0, cosPad);
    native.dawnWriteBuffer(this.backend, s.sinBuf, 0, sinPad);
    ...
    native.dawnWriteBuffer(this.backend, s.acc, 0, accInit);

    this._tEncStart = performance.now();
    const enc = native.dawnBeginEncoder(this.backend);

    // Dawn binding helper. Bindings are buf handles in slot order
    ...
    const dispatchVk = (name, pushData, bufBindings, grid) => {
      native.dawnEnqueueDispatch(enc, name, bufBindings, pushData, grid);
    };
    ...
    const out = native.dawnSubmitAndReadback(enc, s.logitsOut, 0, T * cfg.numClasses * 4);
  ...
  dispose() {
    for (const v of this.weights.values()) native.dawnReleaseBuffer(this.backend, v.ptr);
    if (this.scratch) {
      for (const ptr of Object.values(this.scratch)) native.dawnReleaseBuffer(this.backend, ptr);
    }
    native.dawnDestroyBackend(this.backend);
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  fwd.dispose();
  console.log("[DawnForward] disposed cleanly");
}
